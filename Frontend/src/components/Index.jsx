...
export default function Index()
{
  ...
  const getTodo = async (token) => {
    ...
    return axios.get(`http://localhost:8080/justificatives`, {
  ...
  const get = async (id, token) => {
    return axios.get(`http://localhost:8080/justificatives/${id}`, {
      headers: { 'Authorization': `Bearer ${token}` }
    }).then(response => {
      console.log(response.data);
    })
  ...
  const post = async (token) => {
    return axios.post(`http://localhost:8080/justificatives`,{
